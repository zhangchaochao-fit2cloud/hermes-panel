...
const COVERAGE: Record<string, CoverageMeta> = {
  ...
  login: { group: 'config', coverage: 'ready', route: '/settings#providers', example: 'hermes login openai' },
  logout: { group: 'config', coverage: 'ready', route: '/settings#providers', example: 'hermes logout openai' },
  ...
  pairing: { group: 'ops', coverage: 'ready', route: '/channels', example: 'hermes pairing list' },
  ...
  uninstall: { group: 'ops', coverage: 'partial', route: '/developer#cli-parity', example: 'hermes uninstall', risk: 'destructive', fallback: 'hermes uninstall --help' },
...
const HELP_SECTION_BOUNDARY = /^\s*(options|flags|global options|global flags|local flags|examples|arguments|aliases|additional help topics|help topics|environment variables):/i;
