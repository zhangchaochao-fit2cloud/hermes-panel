...
function mixColor(hex: string, target: string, weight: number): string {
  ...
  const mixed = [
    channel(sourceRgb.r, targetRgb.r),
    channel(sourceRgb.g, targetRgb.g),
    channel(sourceRgb.b, targetRgb.b),
  ];
  return `#${mixed.map((v) => v.toString(16).padStart(2, '0')).join('')}`;
}

export function createPanelThemeOverrides(options: {
  primary?: string;
  dark: boolean;
}): GlobalThemeOverrides {
  const primary = options.primary || FALLBACK_PRIMARY;
  const hover = mixColor(primary, '#ffffff', options.dark ? 0.18 : 0.12);
  ...
  return {
    ...
    Popover: {
      color: options.dark ? '#171717' : '#ffffff',
      textColor: options.dark ? '#fafafa' : '#1f2937',
      borderRadius: '8px',
    },
  };
}
